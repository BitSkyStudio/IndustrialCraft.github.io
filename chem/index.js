...
const createSettings = () => {
...

  {
    ...
    startButton.onclick = () => {
      window.localStorage.setItem(
        "chem.settings.useBGroup",
        document.getElementById("useBGroup").checked
      );
      ...
      for (let i = 0; i < 20; i++) {
        ...
        while (
          chemicalElements[randomElement].b &&
          window.localStorage.getItem("chem.settings.useBGroup") == "false"
        ) {
      ...
      for (let elem in chemicalElements) {
        const element = document.getElementById(
          "periodicTableElement" +
            chemicalElements[elem].x +
            "_" +
            chemicalElements[elem].y
        ).firstChild;
      ...
      for (let e of document.getElementsByClassName("periodicTableElement")) {
        if (
          e.firstChild !== null &&
          e.firstChild.dataset !== undefined &&
          e.firstChild.dataset.elementId === undefined
        )
          e.firstChild.remove();
      }
      document.getElementById("periodicElementName").innerText =
        chosenElements[0];
...
const elementQuizTick = () => {
  ...
  if (timer <= 0) {
    ...
    const correctAnswear =
      beforeInput.value.toLowerCase() ===
      beforeInput.dataset.elementId.toLowerCase()
        ? null
        : beforeInput.dataset.elementId;
    beforeInput.style.backgroundColor =
      correctAnswear === null ? "green" : "red";
    if (correctAnswear !== null)
      beforeInput.value = beforeInput.value + "(" + correctAnswear + ")";
    elementQuizCurrent++;
...
const periodicElementClick = (x, y) => {
  const clickedElement = document.getElementById(
    "periodicTableElement" + x + "_" + y
  );

  const periodicName = document.getElementById("periodicElementName");
  if (
    clickedElement.firstChild.dataset.elementId ===
    chemicalElements[periodicName.innerText].s
  ) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (
        (window.localStorage.getItem("chem.settings.useBGroup") == "false" &&
          skip) ||
        window.localStorage.getItem("chem.settings.useBGroup") == "true"
      )
